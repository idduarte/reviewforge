...
import type { ExtraDocumentFile, Finding, Participant } from "../domain/reviewTypes";
  ...
  return (
    <>
      <PageHeader
        ...
        title={t("extraDocuments.title")}
        subtitle={t("extraDocuments.subtitle")}
        tiles={[
          { label: t("ui.sectionFiles"), value: String(files.length), sub: "docs" },
          { label: t("ui.findings"), value: String(totalFindings), sub: t("ui.total") },
        ]}
        cta={<button className="rf-primary-btn" type="button" onClick={onAddFile}><PlusIcon /><span>{t("extraDocuments.addFile")}</span></button>}
      />

      {selectedIndex === -1 && (
        <FileListView files={files} onSelect={onSelectFile} onAdd={onAddFile} addLabel={t("extraDocuments.addFile")} formatHint=".pdf · .doc · .xlsx" />
      )}
      ...
      {selectedIndex >= 0 && file && (
        <section className="rf-file-card">
          <div className="rf-file-card-header">
            ...
            <button className="rf-ghost-btn sm" type="button" onClick={() => onAddFinding(selectedIndex)}><PlusIcon size={12} /><span>{t("extraDocuments.addFinding")}</span></button>
            <button className="rf-icon-btn danger" type="button" title={t("extraDocuments.removeFile")} onClick={() => onRemoveFile(selectedIndex)}><TrashIcon /></button>
          </div>
